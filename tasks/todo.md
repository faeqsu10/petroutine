...
- [x] 추천 탭 1차 UX 완성 (더미 카탈로그 + 현재 펫 기준 필터 + 상세 시트 + CTA 상태)
- [x] 사료/용품 추천 실데이터화 1차 (Firestore 카탈로그 + 조회 훅 + 시드)
- [x] 추천 클릭 로그 + 카탈로그 운영 문서 추가
- [x] 추천 운영 화면 관리자 접근 제어
- [ ] 카카오맵 API 연동 (병원 지도 검색) — 승인 대기 중
- [ ] 사료/용품 추천 실제 제휴 링크 연결/관리 — PRD v2
